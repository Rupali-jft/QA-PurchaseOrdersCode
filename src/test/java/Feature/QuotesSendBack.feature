Feature: Quotes are added by Initiator and sentback by Approver

  Scenario: Purchase order quotes send back by approver functionality
   ...
   ###------------Approver - Approve approves the Quotes which is raised by initiator-------------###
    Given I navigate to login page of PO app
    And I enter login credentials
      | email      | password |
      | XXXX| XXXX |
